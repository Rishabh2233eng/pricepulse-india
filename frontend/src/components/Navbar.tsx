    ...
    <>
      <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 100, background: "#1E3A2F", borderBottom: "1px solid #163020" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 24px", height: 54, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {links.map((l) => {
              ...
              return (
                <Link key={l.href} href={l.href}
                  style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 11px", borderRadius: 7, textDecoration: "none", fontSize: 12, fontWeight: 500, color: active ? "#4CAF7D" : "#9A8A6E", background: active ? "rgba(76,175,125,0.12)" : "transparent", transition: "all 0.15s" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ color: "#6B5B3E", fontSize: 11, fontWeight: 500 }} className="desktop-nav">Live</span>
            {/* Hamburger — only on mobile */}
            <button onClick={() => setOpen(!open)}
              style={{ background: "none", border: "1px solid #2D4A3E", borderRadius: 6, cursor: "pointer", color: "#9A8A6E", padding: "5px 8px", display: "none" }}
        ...
        {/* Mobile dropdown */}
        {open && (
          <div style={{ background: "#172E22", borderTop: "1px solid #163020", padding: "8px 24px 16px" }} className="mobile-menu-btn">
            {links.map((l) => {
              const Icon = l.icon;
              return (
      ...
      <div className="mobile-bottom-nav">
        {links.map((l) => {
          ...
          return (
            <Link key={l.href} href={l.href}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, textDecoration: "none", padding: "4px 0", flex: 1 }}>
              <Icon size={19} strokeWidth={2} color={active ? "#4CAF7D" : "#6B5B3E"} />
              <span style={{ fontSize: 9, fontWeight: 600, color: active ? "#4CAF7D" : "#6B5B3E", letterSpacing: "0.03em", textAlign: "center" }}>{l.label}</span>
            </Link>
          );
        })}
      </div>
